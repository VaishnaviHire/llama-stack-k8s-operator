...
`,
			check: func(t *testing.T, cfg *BaseConfig) {
				...
				if len(cfg.Models) != 1 || cfg.Models[0].ModelID != "llama3" {
					t.Errorf("expected registered resource model 'llama3', got %+v", cfg.Models)
				}
				if cfg.Raw["registered_resources"] == nil {
					t.Error("expected raw config to preserve registered_resources")
				}
			},
		},
		{
